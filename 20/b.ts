...
export const solution = (input: string) => {
  ...
  const special = ["dh", "vf", "m", "mk"];

  for (let buttonPresses = 0; buttonPresses < Infinity; buttonPresses++) {
      ...
      if (mod) {
        ...
        for (const { name, pulse } of results) {
          ...
          if (target) {
            ...
            if (name == "xm" && pulse == HIGH) {
              console.log(buttonPresses + 1, mod.name, pulse == LOW ? "LOW" : "HIGH");
            }
