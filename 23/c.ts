...
export const solution = (input: string) => {
  ...
  {
    // let p = 0;
    const positions: [Set<Node>, number, Node][] = [[new Set([startNode]), -1, startNode]];

    while (positions.length) {
      // p++;
      const [pathSet, length, node] = positions.pop()!;
      ...
      for (const edge of node.edges) {
        ...
        if (nextNode == endNode) {
          if (length + edge.length + 1 > endNode.bestDistance) {
            endNode.bestDistance = length + edge.length;
          }
          continue;
        }
        if (!pathSet.has(nextNode)) {
          positions.push([new Set([...pathSet, nextNode]), length + edge.length + 1, nextNode]);
        }
      }
